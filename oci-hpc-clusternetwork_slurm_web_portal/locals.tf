...
// Cluster OCID
  ...
  auth_token = var.web_submission_portal ? var.use_existing_auth_token ? var.auth_token : sensitive(oci_identity_auth_token.auth_token[0].token) : "none"
  api_gw_id = var.web_submission_portal ? var.use_existing_api_gw ? var.api_gw_id : oci_apigateway_gateway.generated_oci_apigateway_gateway[0].id : "none"
  registry_id = var.web_submission_portal ? var.use_existing_registry ? var.registry_id : oci_artifacts_container_repository.container_repository[0].id : "none"
  vault_id = var.web_submission_portal ? var.use_existing_vault ? var.vault_id : oci_kms_vault.ssh_vault[0].id : "none"
  master_key_id = var.web_submission_portal ? var.use_existing_master_key ? var.master_key_id : oci_kms_key.master_key[0].id : "none"
  type_api_gateway = var.private_deployment ? "PRIVATE" : "PUBLIC"
}
