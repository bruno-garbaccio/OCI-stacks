resource "oci_core_vcn" "vcn" {
  ...
  display_name   = "${local.jupyter_name}_VCN"
  dns_label      = "jupyter"
}
...
resource "oci_core_internet_gateway" "ig1" {
  ...
  vcn_id         = oci_core_vcn.vcn[0].id
  compartment_id = var.targetCompartment
  display_name   = "${local.jupyter_name}_internet-gateway"
}
...
resource "oci_core_nat_gateway" "ng1" {
  ...
  vcn_id         = oci_core_vcn.vcn[0].id
  compartment_id = var.targetCompartment
  display_name   = "${local.jupyter_name}_nat-gateway"
}
...
resource "oci_core_service_gateway" "sg1" {
  ...
  vcn_id         = oci_core_vcn.vcn[0].id
  compartment_id = var.targetCompartment
  display_name   = "${local.jupyter_name}_service-gateway"
...
resource "oci_core_route_table" "public_route_table" {
  ...
  compartment_id = var.targetCompartment
  vcn_id         = oci_core_vcn.vcn[0].id
  display_name   = "${local.jupyter_name}_public_route_table"
...
resource "oci_core_route_table" "private_route_table" {
  count          = var.use_existing_vcn ? 0 : 1
  display_name   = "${local.jupyter_name}_private_route_table"
  compartment_id = var.targetCompartment
  vcn_id         = oci_core_vcn.vcn[0].id
...
resource "oci_core_subnet" "public-subnet" {
  ...
  dns_label           = "public"
  display_name        = "${local.jupyter_name}_public_subnet"
...
resource "oci_core_subnet" "private-subnet" {
  ...
  display_name               = "${local.jupyter_name}_private_subnet${count.index+1}"
